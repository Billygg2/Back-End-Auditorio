package ec.edu.unibe.auditorio_backend.domain.repository;

import ec.edu.unibe.auditorio_backend.domain.entity.TipoRequerimientoEntity;
import org.springframework.data.jpa.repository.JpaRepository;
import java.util.List;
import java.util.Optional;

public interface TipoRequerimientoRepository 
        extends JpaRepository<TipoRequerimientoEntity, Long> {

    List<TipoRequerimientoEntity> findByActivoTrue();
    boolean existsByNombreIgnoreCase(String nombre);
    Optional<TipoRequerimientoEntity> findByNombreIgnoreCase(String nombre);
}
